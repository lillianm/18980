...
class MapPanel extends JPanel {
	...
	protected synchronized void paintComponent(Graphics g_) {
		...
		Image bgImage = Toolkit.getDefaultToolkit().getImage("/Users/yima/Desktop/bg2.png");
...
//			Ellipse2D circle = new Ellipse2D.Float((float)point.getLatitude(), (float)point.getLongitude(), 10,10);
...
//		}
		...
		for(POI poi : pois) {
			Ellipse2D circle = new Ellipse2D.Float((float)poi.getLatitude(), (float)poi.getLongitude(), 10,10);
			Shape clip = g.getClip();
			g.setClip(circle);
			g.setColor(Color.BLUE);
			g.fill(circle);
